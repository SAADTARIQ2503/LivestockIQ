...
Algorithm:
  ...
  3. At query time: find bucket + ±5 nearby buckets for candidates
...
class VaccineRecommender:
    ...
    # LSH hyper-parameters (must match values used at index-build time)
    K = 3        # k-gram size
    ...
    @classmethod
    def reset(cls):
        """Force a rebuild on next call (useful after DB migrations)."""
        cls._instance = None

    def recommend(self, query: str, top_n: int = 5, min_score: float = 0.01) -> list[dict]:
        """
        ...
        # --- Step 2: gather candidates from bucket ± 5 ---
        n_buckets = 2 ** self.N_BITS
        search_range = range(
            max(0, query_bucket - 5),
            min(n_buckets, query_bucket + 6)
        )
        candidates = self.df[self.df["lsh_bucket"].isin(search_range)].copy()
    ...
    def _build(self):
        ...
        if not records:
            # Nothing in DB yet – index will be built lazily later
            return
        ...
        # Fit global TF-IDF once on the full corpus
        self.tfidf = TfidfVectorizer(stop_words="english")
        self.tfidf.fit(df["search_content"])

        self._ready = True

    def _get_kgrams(self, text: str) -> list[str]:
        text = str(text).lower()
        k = self.K
        if len(text) < k:
            return [text]
    ...
    def _build_lsh_index(self, df: pd.DataFrame):
        ...
        # Build vocabulary from all shingles
        ...
        # Deterministic random vectors (same seed as notebook)
